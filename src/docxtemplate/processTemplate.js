...
const extractQuery = async (template, options) => {
  const ctx = {
    fCmd: false,
    cmd: '',
...
const produceJsReport = async (data, template, options) => {
...
const processText = async (data, node, ctx) => {
...
const processEndForIf = (data, node, ctx, cmd, cmdName, cmdRest) => {
  const curLoop = getCurLoop(ctx);
...
/* eslint-disable */
const processImage = async (ctx, imagePars) => {

  console.log('PROCESSING IMAGE:', imagePars)
...
const getImageData = async (imagePars) => {
