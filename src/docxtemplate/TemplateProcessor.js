...
class TemplateProcessor {
  ...
  async extractQuery(template, options) {
    ...
    while (true) {
      ...
      if (
        ...
        await this.processText(null, nodeIn, ctx);
      }
  ...
  async produceJsReport(data, template, options) {
    ...
    while (true) {
      ...
      if (move === 'DOWN' || move === 'SIDE') {
        ...
        if (
          ...
          newNodeAsTextNode._text = await this.processText(data, nodeIn, ctx);
        }
  ...
  async processText(data, node, ctx) {
    ...
    for (let idx = 0; idx < segments.length; idx++) {
      ...
      if (idx < segments.length - 1) {
        if (ctx.fCmd) {
          const cmdResultText = await this.processCmd(data, node, ctx);
  ...
  // async getImageData(imagePars) {

  //   console.log('IMAGE PAIRS:', imagePars)
  ...
  //   return await this.onReadImage(imagePars)

  //   /*
  //       const URIs = await this.fileService.getFilesURI(imagePars.id)
  //       const imageURI = URIs[0]

  //       // const exists = await this.fileService.isFileExist(imageURI)

  //       // if (!exists) {
  //       //   console.error(`FILE ${imageURI} DOES NOT EXIST`)
  //       //   throw new Error(`FILE ${imageURI} DOES NOT EXIST`)
  //       // }

  //       const extenions = await this.fileService.getFileInfos(imageURI)
  //       const ext = extenions[0].type

  //       const base64Buffer = await this.fileService.readFile(imagePars.id)

  //       return { extension: ext, data: base64Buffer }
  //   */
  ...
  //   // const exists = await this.fileService.isFileExist(imgPath)

  //   // if (!exists) {
  //   //   console.error(`FILE ${imgPath} DOES NOT EXIST`)
  //   //   throw new Error(`FILE ${imgPath} DOES NOT EXIST`)
  //   // }
  //   // console.log(`IMAGE EXISTS: ${imgPath} loading...`)
  //   // const base64Buffer = await this.fileService.readFile(imgPath);

  //   // const ext = this.fileService.getExtension(imgPath).toLowerCase()
  ...
  // Command processor
  // ==========================================
  async processCmd(data, node, ctx) {
    ...
    try {
      ...
      if (cmdName === 'QUERY' || cmdName === 'CMD_NODE') {
        ...
        out = await this.processForIf(data, node, ctx, cmd, cmdName, cmdRest);

        // END-FOR
        // END-IF
      } else if (cmdName === 'END-FOR' || cmdName === 'END-IF') {
        out = this.processEndForIf(data, node, ctx, cmd, cmdName, cmdRest);
        ...
        if (!isLoopExploring(ctx)) {
          ...
          if (img != null) await this.processImage(ctx, img);
        }
  ...
  // Individual commands
  // ==========================================
  async processForIf(data, node, ctx, cmd, cmdName, cmdRest) {
  ...
  processEndForIf(data, node, ctx, cmd, cmdName, cmdRest) {
    const curLoop = getCurLoop(ctx);
  ...
  /* eslint-disable */
  async processImage(ctx, imagePars) {

    const { width = 15, height = 10 } = imagePars
    ...
    const relId = `img${id}`;
    ctx.images[relId] = await this.onReadImage(imagePars);
